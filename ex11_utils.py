...
def is_neighbor(point1: Point, point2: Point) -> bool:
    ''' checks whether two points are neighbors on a grid.
        returns true if so
    '''
    y1, x1 = point1
    y2, x2 = point2
    #check that different but that their coords within 1 of each other
    return point1 != point2 and abs(y2 - y1) < 2 and abs(x2 - x1) < 2

def get_word(board: Board, path: Path) -> str:
    '''converts a path on the board to a word 
    according to the letters at each location along the path.
...
def board_coords(board: Board) -> Path:
    '''create a list of tuples representing a list of all 
    coordinates on the board
    '''
    return [(i, j) for i, row in enumerate(board) for j in range(len(row))]

def get_neighbors(point: Point) -> Path:
    '''return a list of tuples of all neighbours within bounds
    '''
    ...
    return [
        ...
        for i in [y - 1, y, y + 1]
        for j in [x - 1, x, x + 1]
        #don't add coords out of bounds 
        if (i, j) != point and in_bounds(i) and in_bounds(j)
    ]

# def path_combinations(board: Board, n=1) -> Generator[Path, any, any]:
''' returns a list of tuples representing a all the
        possible paths of length n on a Boggle board.
'''
    #base: return all the points on the board
    if n < 2:
#         return ([point] for point in board_coords(board))
#     #nested generatot comprehension
    #add every possible neighbour except those already in path
    return (
#         curr_path + [point]
...
def word_combinations(board: Board, n=1) -> list[Path]:
    '''returns a list of tuples representing all the
     paths which give a word of length n in the dict.
    '''
    paths: list[Path] = [[]]
    for i in range(n):
        new_paths = []
...
def find_all_words(board: Board, words: Iterable[str]) -> PathDict:
    """write a generator comprehension which goes through entire board """
    #call the path finder object
...
def is_valid_path(board: Board, path: Path, words: Iterable[str]) -> Optional[str]:
    ''' checks if a word based on a path in boggle board is 
    in a list of wrods. if so, returns the word as as string, 
...
def find_length_n_words(n: int, board: Board, words: Iterable[str]) -> list[Path]:
    ''' returns a list of tuples representing all the
        possible paths of  on a Boggle board, which represent words 
        of length n in the word list
    '''
    words = set(words)
    return [
